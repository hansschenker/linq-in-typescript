...
- `src/async/` — the `./async` subpath: `AsyncEnumerable` over `AsyncIterable`, same pipe/operator-factory structure (`async function*` + `for await...of`), with **full operator parity** to the sync module. Not re-exported from the root (operator names would collide). Async differences: all callbacks may be async (`Awaitable<T>`), second sequences accept `AnyIterable<T>`, `zip` pulls both sides via `Promise.all`, `orderBy` precomputes keys (async key selectors, sync comparers), support types are `AsyncGrouping`/`OrderedAsyncEnumerable`. Home of time-based operators (`debounceTime`), impossible in sync pull; it works by pumping the source eagerly (push inside) and serving the consumer from an internal queue.
- Async ports are tested in grouped files (`test/async-operators/-grouping/-ordering/-terminals.test.ts`) rather than one file per operator; async variants are documented on `docs/async-enumerable.md` rather than per-operator pages (the sync pages stay canonical for semantics).
